...
void dump_histo_img(unsigned int* histo, unsigned int height, unsigned int width, const char *filename)
{
    ...
    for (y = 0; y < height; ++y)
    {
        for (x = 0; x < width; ++x)
        {
 	    if (histo[y * width + x] > 255)
		histo[y * width + x] = 255;
            unsigned char value = (char)histo[y * width + x];
