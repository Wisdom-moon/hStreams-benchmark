...
void LBM_loadObstacle( LBM_Grid grid) {
	int x,  y,  z;

	for( z = 0; z < SIZE_Z; z++ ) {
		for( y = 0; y < SIZE_Y; y++ ) {
			for( x = 0; x < SIZE_X; x++ ) {
				if( RandFloat (0.0f, 1.0f) > 0.5f ) SET_FLAG( grid, x, y, z, OBSTACLE );
			}
		}
	}
...
void LBM_storeVelocityField( LBM_Grid grid ) {
	int x, y, z;
	OUTPUT_PRECISION rho, ux, uy, uz;

	FILE* file = fopen( "lbm.out", "wb" );
...
void LBM_compareVelocityField( LBM_Grid grid) {
	int x, y, z;
	float rho, ux, uy, uz;
